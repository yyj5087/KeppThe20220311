...
import com.example.keppthe20220311.adapters.StartPlaceSpinnerAdapter
...
class EditAppointmentActivity : BaseActivity() {
    ...
    //    약속 장소 관련 멤버변수.
    var marker: Marker? = null //지도에 표시될 하나의 마커. 처음에는 찍지 않는 상태
    ...
    //    내 출발 장소 목록
    val mStarPlaceList = ArrayList<PlaceData>()
...
//                몇번째 아이템이 선택 되었는지, p2 or position 가 알려줌
                    ...
                    mSelectedStartPlace = mStarPlaceList[position]
...
//            장소를 선택했는지? 안했다면 등록 거부.
                ...
                val selectedStartPlace =
                    mStarPlaceList[binding.startPlaceSpinner.selectedItemPosition]   // 지금선택되어있는 아이템이 몇번째 아이템인지.
...
//                   연/월/일은, JAVA/ KOTLIN 언어의 기준 (0~11월)으로 월 값을 줌. 사람은 (1월~12월)
//                    주는 그대로 Calendar에 set하게되면, 올바른 월로 세팅됨

                        mSelectedAppointmentDateTime.set(year, month, dayOfMonth) // 연월일 한번에 세팅 함수.
//                약속일자의 문구를 22/03/88 등의 형식으로 바꿔서 보여주자.
...
//        시간 선택 텍스트뷰 클릭 이벤트 - Ti,ePickDialog
...
//            지도 시작지점 : 학원 위/경도
        ...
        val coord = LatLng(37.577854625935394, 127.0335928182283)
...
//                coord ~ 선택한 latLng 까지 대중교통 경로를 그려보자. (PathOverlay 기능 활용) + ODSay 라이브러리 활용
            ...
            val myODsayService =
                ODsayService.init(mContext, "huoblO28sQW3DqSI1pNoJceF/YteXjSoM4yd8/zRotc")
            ...
            myODsayService.requestSearchPubTransPath(
                ...
                object : OnResultCallbackListener {
                    override fun onSuccess(p0: ODsayData?, p1: API?) {
                        ...
                        val pathArr = resultObj.getJSONArray("path") // 여러 추천 경로중 첫번째만 사용해보자
...
//                                둘려보려는 경로가, 정거장 목록을 내려준다면 (지하철 or 버스) => 내부 파싱
            ...
            apiList.getRequestMyPlaceList().enqueue(object : Callback<BasicResponse> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        mStarPlaceList.clear()

                        mStarPlaceList.addAll(br.data.places)
